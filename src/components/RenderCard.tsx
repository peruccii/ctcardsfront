...
const RenderCard = (
  ...
  if (date) {
    switch (type) {
      case 'LOVE':
        const daysTogether = calculateDaysBetween(String(date));
        dynamicMessage = ` ${daysTogether ? `Juntos há ${daysTogether} dias` : ''} `;
        break;
      case 'BESTFRIENDS':
        const friendshipDays = calculateDaysBetween(String(date));
        dynamicMessage = ` ${friendshipDays ? `Amizade de ${friendshipDays} dias` : ''}`;
        break;
      case 'BIRTHDAY':
        const daysAlive = calculateDaysBetween(String(date));
        dynamicMessage = ` ${daysAlive ? `Parabéns! 🎉 Você já viveu ${daysAlive} dias incríveis!` : ''} `;
        break;
      default:
        dynamicMessage = 'Juntos há 3.455 dias';
    }
  }
        ...
        >
          ...
          <div className="card-container">
            <div className="card">
              ...
              <div className="details">
                <h4 className="color1 items-center"> Querida Angie,</h4>
                <h4 className="color2 margin">
                  {' '}
                  Uma história que vai além das telas
                </h4>
                Cada cena ao seu lado nos filmes sempre me mostrou o quão
                perfeita é nossa conexão, mas a realidade superou qualquer
                roteiro. Você é minha protagonista, a estrela que ilumina minha
                vida. Mal posso esperar para criar mais histórias juntos. Com
                amor, Brad
              </div>
            </div>
          </div>
